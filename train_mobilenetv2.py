...
tb_logger = TensorBoardLogger(config.DIR_LOG, name=config.NAME_MOBILENETV2)
...
model = Unet_CD_Sentinel2(
    encoder_name=config.ENCODER_NAME_MOBILENETV2,
...
checkpoint_callback = pl.callbacks.ModelCheckpoint(
    dirpath=config.DIR_ROOT_MOBILENETV2,
...
trainer = pl.Trainer(
    ...
    precision="16-mixed",
    logger=tb_logger,
    default_root_dir=config.DIR_ROOT_MOBILENETV2,
)
...
smp_model.save_pretrained(config.DIR_ROOT_MOBILENETV2 + "/" + config.NAME_MOBILENETV2)
